...
- (void)reloadData:(NSInteger)index{
    ...
    for (int i = 0; i < total; i++) {
        ...
        if (buttonSize.width > CGRectGetWidth(self.bounds)) {
            ...
            if (i == 0) { //第一个标签，不留间距
                ...
                newRect.origin = CGPointMake(self.lineSpacing ,0);
            ...
            if (CGRectGetMaxX(previousFrame) + buttonSize.width + self.itemMargin > CGRectGetWidth(self.bounds)) {
                newRect.origin = CGPointMake(self.lineSpacing, CGRectGetMaxY(previousFrame) + self.lineSpacing);
            }
            else {
                if (i == 0) { //第一个标签，不留间距
                    newRect.origin = CGPointMake(self.lineSpacing, 0);
    ...
    //默认选中第一个标签
    [self tagButtonTouched:self.allButtons[index]];
}
 ...
 *  删除某个选中标签的选中状态
 */
-(void)changeSomeoneSelectedState{
    
    //删除某个选中标签的选中状态
    [self tagButtonTouched:self.allButtons[[[self indexesOfSelectionTags][0] intValue]]];
}
