...
- (void)drawRect:(CGRect)rect {
    ...
    //------willToucheHyperlinkText------------------设置段落属性-----------------------------
    //指定为对齐属性
...
//    CGSize size = [self sizeThatFits:CGSizeMake(WIDTH - 16, MAXFLOAT)];
...
+(CGFloat)sm_getStringHeightWithString:(NSString * _Nonnull)text
    ...
    if (linesArray.count!=0) {
        line = (__bridge CTLineRef) [linesArray objectAtIndex:[linesArray count]-1];
