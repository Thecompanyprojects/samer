...
@implementation RewardRankingCell
...
-(void)setModel:(RewardRankingModel *)model{
    ...
    self.amount.text = [NSString stringWithFormat:@"%@",model.allamount];
    ...
    if (_model.wealth_val.length == 0) {
        ...
        [self.button1 sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",_model.rewarduserinfo[@"head_pic"]]] forState:UIControlStateNormal placeholderImage:[UIImage imageNamed:@"默认头像"]];
        ...
        [self.button1 sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",_model.rewardeduserinfo[@"head_pic"]]] forState:UIControlStateNormal placeholderImage:[UIImage imageNamed:@"默认头像"]];
    }
...
- (void)awakeFromNib {
    ...
    self.button1.layer.cornerRadius = 18;
